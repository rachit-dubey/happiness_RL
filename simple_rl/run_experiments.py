...
def run_single_agent_on_mdp(agent, mdp, episodes, steps, experiment=None, verbose=False, track_disc_reward=False, reset_at_terminal=False, resample_at_terminal=False, counterfactual_regret=True):
    '''
    Summary:
    ...
    for episode in range(1, episodes + 1):
        ...
        for step in range(1, steps + 1):
            ...
            if counterfactual_regret:
                action = agent.act(state, reward, counterfactual_state, counterfactual_reward) #this only works for happy-q-learning agent
            else:
                action = agent.act(state, reward)#for all other agents
                
            if counterfactual_regret:
                counter_action = agent.counterfactual_policy(action) #obtain the counterfactual action by a random agent
            
            # Terminal check, we don't worry about this for now
            if state.is_terminal():
                ...
                if episodes == 1 and not reset_at_terminal and experiment is not None and action != "terminate":
                    ...
                    experiment.add_experience(agent, state, action, 0, state, time_taken=1)
                    continue
                break
            ...
            if experiment is not None:
                ...
                experiment.add_experience(agent, state, action, reward_to_track, next_state, time_taken=1)
