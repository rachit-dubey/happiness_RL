...
class Experiment(object):
    ...
    def __init__(self,
        ...
        Args:
            ...
            track_success (bool)
            success_reward (int)
            env_num = environment number to append to the directory name
        '''
        ...
        self._setup_files(env_num, clear_old_results)
    ...
    def _write_extra_datum_to_file(self, agent, datum, datum_name):
    ...
    def add_experience(self, agent, state, action, reward, happiness, next_state, time_taken=0):
        '''
        Args:
        ...
        if self.steps_since_added_r % self.count_r_per_n_timestep == 0:
            if self.is_markov_game and self.count_r_per_n_timestep > 1:
                ...
                self.happiness[agent] += [self.hap_since_count + happiness]
            ...
            self.hap_since_count += happiness
            self.steps_since_added_r += 1
    ...
    def end_of_episode(self, agent, num_times_to_write=1):
        ...
        if self.is_episodic:
            ...
            for x in range(num_times_to_write):
                ...
                for step_happiness in self.happiness[agent]:
                    self._write_extra_datum_to_file(agent=agent, datum=step_happiness, datum_name="happiness")    
        self.rewards[agent] = []
