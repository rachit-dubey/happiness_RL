...
cmap = cm.get_cmap('plasma',26) #change number here
...
def plot(results, experiment_dir, agents, plot_file_name="", conf_intervals=[], use_cost=False, cumulative=False, episodic=True, open_plot=True, track_disc_reward=False, add_legend=True):
    ...
    for i, agent_name in enumerate(agents):
        ...
        pyplot.plot(x_axis, y_axis, color=series_color, linewidth=0.5, marker=series_marker, markevery=marker_every, label=agent_name)
        #pyplot.plot(x_axis, y_axis, color=series_color, linewidth=1, label=agent_name)
        if add_legend:
