...
class MDP(object):
    ...
    def simulate_agent_action(self, action):
        '''
        Args:
            action (str)
        ...
        Summary:
            This is for hypothetical/counterfactual regret, the goal is to take the mdp 
            and return what r, s would be for some hypothetical action *without* updating or disturbing the mdp being run.
        '''
        next_state = self.transition_func(self.cur_state, action)
        reward, is_teleportation, new_init_state = self.reward_func(self.cur_state, action, next_state)
    ...
    def execute_agent_action(self, action):
        ...
        if is_teleportation: #if we want teleportation, then agent is teleported after reaching goal location
            self.cur_state =  new_init_state
        else:
